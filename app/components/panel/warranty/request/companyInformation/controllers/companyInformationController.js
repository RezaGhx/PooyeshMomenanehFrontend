companyInformationController.$inject = [
	"panel.warranty.request.companyInformationServices",
	"Upload",
	"$state",
	"$auth",
	"$scope",
...
function companyInformationController(
	companyInformationServices,
	upload,
	state,
	auth,
	scope,
	...
	const getCompany = function () {
		...
		self.promiseLoading = companyInformationServices.get(query).$promise.then(
			response => {
				...
				// self.company.activityType = activityTypeFactory(self.company.activityType);
				// self.company.registerDate = dateFactory(self.company.registerDate);
				// self.company.companyRegistrationType = companyTypeFactory(self.company.companyRegistrationType);
			},
			errResponse => {
				console.log("error");
			}
		...
		if (form.$valid) {
			...
			companyInformationServices.update(parameter, company).$promise.then(
				response => {
					...
					state.go("panel.warranty.request.registryInformation");
				},
				errResponse => {
					iziToast.show({
...
module.exports = ngModule => {
	ngModule.controller(
		"panel.warranty.request.companyInformationController",
