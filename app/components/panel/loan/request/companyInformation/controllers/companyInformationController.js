companyInformationController.$inject = ["panel.loan.request.companyInformationServices", "Upload", "$state", "$auth", "activityTypeFactory", "$scope", "companyTypeFactory", "ProvinceFactory", "CityFactory"];
		...
		if (form.$valid) {
			...
			companyInformationServices.update(parameter, company).$promise.then(
				response => {
					...
					state.go("panel.loan.request.registryInformation");
				},
				errResponse => {
					iziToast.show({
...
module.exports = ngModule => {
	...
	ngModule.controller('panel.loan.request.companyInformationController', companyInformationController);
