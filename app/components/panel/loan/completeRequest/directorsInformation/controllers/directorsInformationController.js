directorsInformationController.$inject = [
  "completeRequest.directorsInformationServices",
  "$state"
];

function directorsInformationController(directorsInformationServices, state) {
  var self = this;
  self.member = {
    shareholderInfo: []
  };
  ...
  self.deleteItem = async function (item) {
    ...
    await directorsInformationServices.remove(parameter).$promise.then(data => {
      self.company.directorMembers.splice(self.company.directorMembers.indexOf(item), 1);
    }, errResponse => {
      iziToast.show({
        message: errResponse.data.message,
        theme: "light",
  ...
  self.deleteShareholdersItem = async function (item) {
    ...
    await directorsInformationServices.remove(parameter).$promise.then(data => {
      self.companyMember.shareholdersMembers.splice(self.companyMember.shareholdersMembers.indexOf(item), 1);
    }, errResponse => {
      iziToast.show({
        message: errResponse.data.message,
        theme: "light",
...
module.exports = ngModule => {
  ngModule.controller(
    "completeRequest.directorsInformationController",
