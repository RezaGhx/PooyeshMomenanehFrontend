companyInformationController.$inject = ["editRequest.companyInformationServices", "Upload", "$state", "$auth", "activityTypeFactory", "$scope", "companyTypeFactory", "ProvinceFactory", "CityFactory"];
    ...
    if (form.$valid) {
      ...
      companyInformationServices.update(parameter, company).$promise.then(
        response => {
          ...
          state.go("panel.loan.editRequest.registryInformation");
        },
        errResponse => {
          iziToast.show({
...
module.exports = ngModule => {
  ...
  ngModule.controller('editRequest.companyInformationController', companyInformationController);
