indexController.$inject = ["panel.jahadiPanel.indexServices", "Upload", "$state", "$timeout"];

function indexController(indexServices, upload, state, timeout) {
    var self = this;
    ...
    self.submit = function (ticket, form) {
        if (form.$valid) {
            ...
            indexServices.create(parameter, ticket).$promise.then(
                response => {
                    ticket = {};
                    form.$setUntouched();
                    form.$setPristine();
...
module.exports = ngModule => {
    ngModule.controller("panel.jahadiPanel.indexController", indexController);
};
